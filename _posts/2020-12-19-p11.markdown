...
虽然对于计算语言学的研究者们来说，这些分支的区别很大，但对计算机科学背景的研究者来讲，这些区别并不重要，所有的生成语法都是可以用程序语言描述的，而恰好HPSG则是这其中有相对完善的软件支持的语法[2]。
...
在HPSG，描述这样的限制是靠两个相同的索引完成的([6])，这样的索引可以放置在任何结构和任何位置。<br><br>

而ARG-ST则是用来描述这个词在句子里完整而且必要语境，其作用和语言模型非常类似。
...
但在long term dependency的情况下，specifier或者complements可以被省略掉，假设省略掉的部分已经在之前出现过。
...
与计算机科学里的面向对象编程像似，HPSG里的lexical entry可以继承自一个更笼统的父类，然后再通过lexical rule进行转变，
...
上面的两幅图简单地描述了句法分析的过程，其中的动词continues承担了整个句子的中心词，所以整个句子的索引(INDEX)和continues的索引是一样的(都为s1)。
...
SEM值里的RESTR是包括了所有终端节点的实体关系组（predication)，而且RESTR是一个集合的概念，
...
从计算语言学家的角度来看，任何语言都包含了大量看起来随机而且没有规律的东西，
而语言形式和语义之间的联系很大程度上是惯例决定的(自古以来的用法)。
一个能把从语言形式(form)转换到语义(semantics)的系统必须要记录大量的琐碎的知识，
...
HPSG对语法正确性的要求更是让其实用性受到致命的打击，
这也是我认为符号主义(symbolism)的系统不具有可以独立挑战深度学习的重要原因。
但是从另外一个观点看，HPSG这种对形式和语义的清晰分层，是深度学习难以做到的。
深度学习端对端的思路从形式作为输入开始，一直到特定应用的决策(例如问答系统的多选题)输出，
中间并没有允许语义层存在的空间，更不用说语义层上的语用层(pragmatics)。
这样的学习是非常低效，这也助长了近年来热门的神经网络越做越大的思路。
如何把语义层和语用层植入语言理解系统，一定是解开自然语言理解的必经之路。
