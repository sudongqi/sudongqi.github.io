...
It's also common to solve PCA problem with singular value decomposition. First, let us apply SVD on $$X$$:
...
Given (7) and (8), it's not hard to see that $$m P = V$$ and $$m D = \Sigma^2$$, where $$m$$ is a constant. 
